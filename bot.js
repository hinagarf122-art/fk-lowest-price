...
bot.onText(/\/deny (.+)/, async (msg,m) => {
  ...
  tg(ADMIN_CHAT_ID,`❌ <code>${uid}</code> denied.`);
  tg(uid,`❌ Access request denied.`);
});
...
bot.onText(/\/addproduct (.+)/, async (msg,m) => {
  ...
  if (db.products.find(p=>p.url===url)) return tg(cid,'⚠️ Already tracking!');
  await tg(cid,'⏳ Fetching live price from Flipkart...');
  const info = await scrapeFK(url);
  if (!info) return tg(cid,'❌ Could not fetch. Check URL and try again.');
...
bot.onText(/\/status/, async msg => {
  ...
  tg(cid,
    `📊 <b>Status</b>\n\n`+
    `🔄 Checking: ${db.isChecking?'✅ Active':'❌ Stopped'}\n`+
...
// ─── FLIPKART SCRAPER (axios + cheerio, NO puppeteer) ─────────────────────────
const FK_HEADERS = {
...
function parsePrice(str) {
  if (!str) return null;
  const m = str.replace(/[,\s]/g,'').match(/(\d{3,6})/g);
  if (!m) return null;
  // Pick the largest reasonable number (actual price)
  const nums = m.map(Number).filter(n=>n>=100&&n<=500000);
...
async function scrapeFK(url) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      console.log(`[Scraper] Attempt ${attempt}: ${url.slice(0,60)}`);
      ...
      // ── Product Name ─────────────────────────────────────────
      let name = '';
      const nameSelectors = ['span.VU-ZEz','span.B_NuCI','h1._9E25nV span','h1 span','._35KyD6','.yhB1nd'];
      for (const s of nameSelectors) {
        const t = $(s).first().text().trim();
        if (t && t.length > 5) { name = t; break; }
      }
      ...
      // Fallback: scan all elements with ₹
      if (!price) {
        $('*').each((_,el) => {
      ...
      const lowestSelectors = ['.yRaY8j.ZYYwLA','.yRaY8j','._3qQ9m1','.PCWT0u','.LFwuGS'];
      ...
      // Fallback: search "lowest" text context
      if (!lowestPrice) {
        const bodyText = $('body').text();
        const lowestMatch = bodyText.match(/lowest price[^₹]*₹[\s]*([\d,]+)/i);
        if (lowestMatch) lowestPrice = parsePrice(lowestMatch[1]);
      }

      // Fallback: bank offer price (lower than main price)
      if (!lowestPrice && price) {
        $('[class*="offer"],[class*="bank"],[class*="Bank"]').each((_,el) => {
          if (lowestPrice) return false;
          const t = $(el).text();
          if (t.includes('₹')) {
            const p = parsePrice(t);
            if (p && p < price && p > 100) lowestPrice = p;
          }
        });
      }
      ...
      if (!price) {
        console.log(`[Scraper] Attempt ${attempt}: no price in HTML`);
        await new Promise(r=>setTimeout(r,3000));
        continue;
      }

      console.log(`[Scraper] ✅ ${name.slice(0,40)} | ₹${price} | Lowest: ₹${lowestPrice}`);
...
function startChecking() {
  ...
  console.log('✅ Price check loop started (15s)');
...
async function runCheck() {
  ...
  for (const p of db.products) {
    try {
      ...
      const priceChanged  = info.price       !== p.price;
      const lowestChanged = info.lowestPrice !== p.lowestPrice;
...
// ─── KEEP-ALIVE ───────────────────────────────────────────────────────────────
setInterval(() => {
  const url = RENDER_URL.startsWith('https') ? `${RENDER_URL}/ping` : `http://localhost:${PORT}/ping`;
  try {
    const mod = url.startsWith('https') ? https : http;
    const r = mod.get(url, ()=>{});
    r.on('error',()=>{}); r.setTimeout(5000,()=>r.destroy());
  } catch(e){}
}, 25000);
...
.sc:hover{border-color:#2563eb55;transform:translateY(-2px);box-shadow:0 6px 20px rgba(0,0,0,.35)}
.si{font-size:30px;flex-shrink:0}
...
.ba{background:var(--acc);color:#fff}.ba:hover{background:#1d4ed8;box-shadow:0 3px 10px rgba(37,99,235,.4)}
...
.tg2{background:rgba(16,185,129,.12);color:#6ee7b7;border:1px solid rgba(16,185,129,.2)}
...
<nav>
  <div class="brand">
    ...
    <div><h1>Flipkart Price Alert Bot</h1><small>Real-time · 15s interval · No browser needed</small></div>
  </div>
...
<div class="wrap">
  ...
  <div class="row2">
    <div class="card">
      ...
      <div class="cb">
        ...
        <small style="color:var(--mu)">⚠️ Only flipkart.com links accepted</small>
        <div class="ctrlrow">
  ...
  <div class="card">
    ...
    <div class="cb"><div class="plist" id="plist"><div class="empty"><div class="ei">📭</div><div>No products yet</div></div></div></div>
  </div>
</div>
...
async function loadData(){
  try{
    ...
    document.getElementById('stxt').textContent=on?'Checking Active':'Stopped';
    ...
    <div class="pc">
      ...
      <div class="pi">
        ...
        <div class="ptags">
          ...
          \${p.lowestPrice&&p.lowestPrice!==p.price?\`<span class="tag tg2">🏷 ₹\${fmt(p.lowestPrice)}</span>\`:''}
        </div>
        <div class="pt">Last checked: \${p.lastChecked?new Date(p.lastChecked).toLocaleString('en-IN'):'Never'}</div>
      </div>
      <button class="btn bsm br" onclick="del('\${p.id}')">🗑 Remove</button>
    </div>
  \`).join('');
}
...
async function ctrl(a){
  ...
  const d=await r.json();toast(d.message||'Done');log('✅ '+d.message,'ls');loadData();
}
...
app.post('/api/products', async (req,res) => {
  ...
  const info = await scrapeFK(url);
  if (!info||!info.price) return res.json({success:false,error:'Could not fetch price. Try again.'});
...
app.post('/api/control', (req,res) => {
  ...
  if (action==='start') { startChecking(); return res.json({message:'✅ Checking started'}); }
...
app.listen(PORT, () => {
  ...
  console.log(`🤖 Telegram Bot ACTIVE | Admin: ${ADMIN_CHAT_ID}`);
});
